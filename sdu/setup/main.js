...
{
	if (region.Q.z === region.W.z)
	{
		return 'on line ' + region.Q.z;
	}
	return 'on lines ' + region.Q.z + ' through ' + region.W.z;
}
...
var author$project$Main$AppTracking = 0;
var author$project$Main$Model = F6(
	function (username, users, trackingType, measurementDays, errors, link) {
		return {J: errors, M: link, A: measurementDays, E: trackingType, H: username, I: users};
	});
...
var author$project$Main$init = A6(author$project$Main$Model, '', '', 0, '', _List_Nil, '');
var author$project$Main$trackingTypeToString = function (tType) {
	if (tType === 1) {
		return '0';
	} else {
		return '1';
...
var author$project$Main$mkSetupLink = function (model) {
	var jsonData = '{ \"user_id\": \"' + (model.H + ('\"' + (', \"users\": \"[' + (model.I + (']\"' + (', \"tracking_type\": \"' + (author$project$Main$trackingTypeToString(model.E) + ('\"' + (', \"measurement_days\": \"' + (model.A + '\" }'))))))))));
	var encodedData = truqu$elm_base64$Base64$encode(jsonData);
	return 'sdutracker://?data=' + encodedData;
};
...
var author$project$Main$modelValidator = rtfeldman$elm_validate$Validate$all(
	_List_fromArray(
		[
			A2(
			...
			function (_n0) {
				return 'Indtast et antal måledage.';
			}),
			A2(
			rtfeldman$elm_validate$Validate$ifBlank,
			function ($) {
			...
			'Indtast en liste af brugere (separaret med komma).')
		]));
		...
		switch (msg.$) {
			...
			case 2:
				...
				return _Utils_update(
					...
					{I: newUsers});
			case 3:
				var newTrackingType = msg.a;
				return _Utils_update(
					model,
					{E: newTrackingType});
			case 4:
				var newMeasurementDays = msg.a;
				return _Utils_update(
					model,
					{A: newMeasurementDays});
			default:
				var _n1 = A2(rtfeldman$elm_validate$Validate$validate, author$project$Main$modelValidator, model);
				if (_n1.$ === 1) {
					var errors = _n1.a;
					return _Utils_update(
						model,
					...
					return _Utils_update(
						...
						{
							J: _List_Nil,
							M: author$project$Main$mkSetupLink(model)
						});
				}
		}
	});
var author$project$Main$CreateLink = {$: 5};
var author$project$Main$MeasurementDays = function (a) {
	return {$: 4, a: a};
};
...
var author$project$Main$viewLink = function (link) {
	var _n0 = elm$core$String$isEmpty(link);
	if (_n0) {
		...
		return A2(
			...
			_List_fromArray(
				[
					elm$html$Html$Attributes$class('generated-link')
				]),
			_List_fromArray(
				[
...
var author$project$Main$DeviceTracking = 1;
var author$project$Main$TrackingType = function (a) {
	return {$: 3, a: a};
};
...
var author$project$Main$viewTrackingTypePicker = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text('Tracking type:')
					])),
				A3(
				author$project$Main$radio,
				'App',
				!model.E,
				author$project$Main$TrackingType(0)),
				A3(
				author$project$Main$radio,
				'Tid',
				model.E === 1,
				author$project$Main$TrackingType(1))
			]));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text('Opret Setuplink')
					])),
				A2(
				elm$html$Html$div,
				_List_Nil,
				_List_fromArray(
					[
						A4(author$project$Main$viewInput, 'text', 'Brugernavn', model.H, author$project$Main$Username),
						A4(author$project$Main$viewInput, 'number', 'Antal måledage', model.A, author$project$Main$MeasurementDays),
						A4(author$project$Main$viewInput, 'text', 'Brugere (separeret med komma)', model.I, author$project$Main$Users),
						author$project$Main$viewTrackingTypePicker(model),
						A2(
						elm$html$Html$button,
						_List_fromArray(
							[
								elm$html$Html$Events$onClick(author$project$Main$CreateLink)
							]),
						_List_fromArray(
							[
								elm$html$Html$text('Opret Link')
							]))
					])),
				author$project$Main$viewErrors(model.J),
				author$project$Main$viewLink(model.M)
			]));
};
...
var author$project$Main$main = elm$browser$Browser$sandbox(
	{aA: author$project$Main$init, aL: author$project$Main$update, aN: author$project$Main$view});
